...
이름은 "LLM이 호출한다"지만, **실제로 함수를 실행하는 것은 우리 앱입니다.** LLM은 "이 함수를 이 인자로 불러 달라"고 요청할 뿐입니다. 이 구분이 이 장의 전부입니다.
...
그림에서 LLM 호출이 두 번 일어나는 것이 핵심입니다. LLM이 실행을 못 하기 때문입니다.

- **1차** — 질문과 도구 목록을 보냅니다. LLM은 답 대신 `getWeather("서울")`을 불러 달라는 **요청**을 돌려줍니다(함수명과 인자를 담은 구조화된 출력).
- **앱** — 실제로 그 함수를 실행해 `"맑음, 22도"`를 얻습니다.
- **2차** — 결과를 다시 보냅니다. `"맑음, 22도"`라는 원시 데이터를 사람이 읽을 문장으로 바꾸는 건 LLM의 몫입니다.

> **비용과 지연이 최소 두 배입니다.** LLM이 도구를 연달아 쓰면 그만큼 더 늘고, 매 호출마다 대화 전체가 다시 올라가므로 토큰도 함께 붙습니다. 사용량을 보는 방법은 §9에서 다룹니다.
