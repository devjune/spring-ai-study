...
Ollama는 로컬에서 모델을 직접 돌리므로 **API 키가 없습니다**(기본 접속지 `http://localhost:11434`). 외부망이 막힌 환경에서 유일하게 동작하는 선택지이기도 합니다.
...
`vectorStore.add(documents)`가 임베딩을 "자동 생성"한다고 했는데, 그 벡터를 만드는 모델은 무엇일까요? **Spring AI에 전역 기본 임베딩 모델이 따로 있는 게 아니라, 클래스패스에 올라온 model 스타터가 정합니다.** 이 예제는 `spring-ai-starter-model-transformers`를 넣었고, 그러면 `TransformersEmbeddingModel`이 자동 구성됩니다.

> "The `TransformersEmbeddingModel` … **locally computes sentence embeddings** using a selected sentence transformer." — Spring AI Reference
>
> "By default, it employs the **all-MiniLM-L6-v2** model, though it is configurable to support other ONNX-compatible models." — `TransformersEmbeddingModel` Javadoc

별도 지정이 없을 때의 동작은 이렇습니다.

- **모델**: `sentence-transformers/all-MiniLM-L6-v2` (384차원, 약 80MB). BERT를 지식 증류로 경량화한 MiniLM 계열(L6 = 트랜스포머 6개 층)로, 문장을 의미가 담긴 384차원 벡터로 바꿔 의미 검색·유사도 비교에 널리 쓰이는 대표 경량 임베딩 모델.
- **로컬 실행**: ONNX 포맷으로 직렬화된 모델을 앱 안에서 직접 돌립니다. **Anthropic 외 추가 API 키가 필요 없는 이유가 이것입니다.**
- **최초 다운로드 + 캐시**: 첫 `embed()` 호출 때 모델(`model.onnx`)과 토크나이저(`tokenizer.json`)를 기본 URI에서 내려받아 로컬에 캐시합니다(temp의 `spring-ai-model-cache…`). 그래서 **첫 기동만 네트워크가 필요하고**, 이후에는 캐시로 오프라인 동작합니다.

바꾸려면 `spring.ai.embedding.transformer` 프로퍼티로 오버라이드합니다.

```properties
spring.ai.embedding.transformer.onnx.model-uri=...     # 다른 ONNX 모델 / 사내 미러
spring.ai.embedding.transformer.tokenizer.uri=...      # 토크나이저 교체 / 미러
spring.ai.embedding.transformer.cache.directory=...    # 캐시 위치
```

> 기본 URI가 외부(githubusercontent)라, 외부망이 막힌 사내망에서는 첫 기동 시 이 다운로드가 막혀 임베딩 초기화가 실패할 수 있습니다. 캐시를 미리 채우거나 위 URI를 사내 미러로 돌리면 됩니다.
...
**임베딩 모델 `all-MiniLM-L6-v2` 계보** (Microsoft MiniLM → 6층 축소 → SBERT 파인튜닝)

- MiniLM 논문 (Microsoft Research, NeurIPS 2020): https://arxiv.org/abs/2002.10957
- MiniLM 공식 저장소 (`microsoft/unilm`): https://github.com/microsoft/unilm/tree/master/minilm
- 6층 파생 모델 `nreimers/MiniLM-L6-H384-uncased`: https://huggingface.co/nreimers/MiniLM-L6-H384-uncased
- 문장 임베딩 모델 `sentence-transformers/all-MiniLM-L6-v2`: https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2
